...
export interface FakeGithubOptions {
  ...
  merged?: boolean;
  mergeStrategyResult?: MergeStrategy;
  nextPrNumber?: number;
  existingPRBranches?: string[];
  reviews?: PullRequestReview[];
  /** Comment body for issue_comment event simulation. */
  commentBody?: string;
  /** Existing comments on the PR (for pull_request_target event simulation). */
  prComments?: string[];
}
...
export class FakeGithub implements GithubApi {
  ...
  private _mergeStrategyResult: MergeStrategy;
  private _nextPrNumber: number;
  private _existingPRBranches: Set<string>;
  private _reviews: PullRequestReview[];
  private _commentBody: string | undefined;
  private _prComments: string[];
  ...
  constructor(options?: FakeGithubOptions) {
    ...
    this._commentBody = options?.commentBody;
    this._prComments = options?.prComments ?? [];
  }
  ...
  async getComments(_issue_number: number): Promise<string[]> {
